...
public java.util.Map<String, Object> getOnRefreshEventObj( ) {
    	...
    	ViewImpl.addPanListener(this, asNativeWidget(), uiView, new ViewImpl.PanCallBack() {
    		...
    		public void handlePanEnd(IWidget widget, Object eventWidget, int x, int y) {
    			if (action == 1) {
    				...
    				action = 0;
    				fragment.getEventBus().notifyObservers("scrollEnabled", Boolean.TRUE);
    			}
    		}
	...
	private boolean processTouchEvent(int action, int x, int y) {
		event.setX(x);
		event.setY(y);
		event.setAction(action);
		return this.customSwipeRefreshLayout.onInterceptTouchEvent(event);
